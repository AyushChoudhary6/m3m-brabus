...
const PUBLIC_PATHS = [
  ...
  "/api/acefone/webhook",
  // ICS calendar feeds — auth'd by an HMAC token in the URL (calendar clients send no
  // cookie), so the PATH must bypass session auth; the handler enforces the token. (#59)
  "/api/calendar.ics", "/api/hr/calendar.ics",
