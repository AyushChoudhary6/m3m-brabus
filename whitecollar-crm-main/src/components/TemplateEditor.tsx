...
interface Template {
  id: string; kind: Kind; trigger: Trigger; name: string; subject: string | null; body: string; visibleToHr?: boolean;
}
...
export default function TemplateEditor({ mode, template }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    body: template?.body ?? "",
    visibleToHr: template?.visibleToHr ?? true, // audit #41 — HR-visible by default
  });
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" {...backdropProps(() => !busy && setOpen(false))}>
          <div className="bg-white rounded-xl max-w-lg w-full p-5 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              {/* Audit #41 — gate Sales-only templates out of the HR recruiter pickers. */}
              <label className="flex items-center gap-2 text-xs text-gray-600">
                <input type="checkbox" checked={form.visibleToHr} onChange={(e) => setForm(f => ({ ...f, visibleToHr: e.target.checked }))} />
                Visible to HR recruiters (untick for Sales-only templates)
              </label>
