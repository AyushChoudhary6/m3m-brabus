...
export function ExpectedJoinings({ items, showOwner }: ExpectedJoiningsProps) {
  return (
    ...
    >
      ...
      {items.length === 0 ? (
        <div className="flex items-center gap-2 px-4 py-3 text-xs text-gray-400 dark:text-slate-500">
          <Inbox className="w-4 h-4 shrink-0" />
          No expected joinings yet — released offers with a joining date show here.
        </div>
      ) : (
