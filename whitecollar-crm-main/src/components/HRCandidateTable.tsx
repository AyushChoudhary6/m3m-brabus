...
// Returns the chip's candidate count, or null for a DERIVED (signal-based) chip
// whose true count can't be known from the server status map — the caller then
// shows no number instead of a misleading "(0)" (audit #22).
function chipCount(key: string, countMap: Record<string, number>, showClosed: boolean): number | null {
  if (key === "all") {
    // "All" must reflect what the list ACTUALLY shows: by default closed
    // candidates are hidden, so exclude them from the count (they're tallied
    // separately under the "Closed" chip). Otherwise "All (3)" appears above a
    // list of 1. When "Show closed" is on, All counts everything.
    return Object.entries(countMap).reduce((a, [k, v]) => a + ((!showClosed && CLOSED_SET.has(k)) ? 0 : v), 0);
  }
  const keys = CHIP_STATUS_KEYS[key];
  if (!keys) return null; // derived/signal chip — no reliable server-side count
...
// Every remaining Excel field, offered as a toggleable column (hidden by default
// via DEFAULT_HIDDEN). [columnKey, header label]. Value getters live in EXTRA_GET
// (below the Candidate type). Keep the two in sync.
const EXTRA_COLS: [string, string][] = [
  ["email", "Email"], ["whatsapp", "WhatsApp"], ["altEmail", "Alt Email"], ["location", "Location"],
  ["source", "Source Portal"], ["notice", "Notice Period"],
  ["salesExp", "Sales Exp"], ["callingExp", "Calling Exp"], ["reExp", "Real Estate Exp"], ["remarks", "Remarks"],
  ["sopMet", "SOP Met"], ["contactedBy", "Contacted By"], ["inviteSent", "F2F Invite Sent"], ["acknowledged", "Acknowledged"], ["arrivedOnTime", "Arrived On Time"],
  ["personality", "Personality"], ["confidence", "Confidence"], ["communication", "Communication"], ["behaviour", "Behaviour"],
  ["grasping", "Grasping Power"], ["listening", "Listening"], ["negotiation", "Negotiation"], ["cultureFit", "Culture Fit"],
  ["highestProduct", "Highest Product Sold"], ["dealValue", "Deal Value"], ["avgIncentive", "Avg Incentive"], ["incentiveConsistent", "Incentive Consistent"],
  ["callsPerDay", "Calls / Day"], ["connectedCalls", "Connected Calls / Day"], ["lastSaleDate", "Last Sale Date"],
  ["hrDecision", "HR Decision"], ["hrRemarks", "HR Detailed Remarks"], ["finalStatus", "Final Status"],
];
...
const COLUMNS: { key: string; label: string; always?: boolean }[] = [
  ...
  ...EXTRA_COLS.map(([key, label]) => ({ key, label })),
  { key: "actions", label: "Actions", always: true },
];
// Columns hidden by default so the table fits ONE screen with no horizontal
// scroll. The kept set (Created, Candidate, Phone, Status, Follow-up, Owner,
// Actions) is the recruiter's at-a-glance view; everything else is one click
// away in the "Columns" menu (which re-enables per-column scroll if needed).
const DEFAULT_HIDDEN = new Set<string>([
  "profile", "exp", "currentSal", "expectedSal", "interview", "lastActivity",
  ...EXTRA_COLS.map(([k]) => k),
]);
// v3: default column set slimmed to fit one screen (no horizontal scroll). The
// version bump makes the new compact default apply over any old saved "show all".
const HIDDEN_COLS_KEY = "hr-candidate-hidden-cols-v3";
...
interface Candidate {
  id: string; name: string; phone: string | null; whatsappPhone: string | null; email: string | null; altEmail: string | null;
  location: string | null; remarks: string | null; currentCompany: string | null; currentProfile: string | null;
  experience: string | null; realEstateExperience: string | null; salesExperience: string | null; callingExperience: string | null;
  currentSalary: number | null; expectedSalary: number | null; noticePeriod: string | null;
  source: string | null; status: string; nextActionDate: string | null; createdAt: string;
...
interface Props {
  ...
  perms?: TablePerms;
  // Whether the server is currently including closed candidates in the list
  // (?closed=1). Drives the "All" chip count so it matches the visible rows.
  showClosed?: boolean;
}
...
type ColKey = "createdDate" | "phone" | "profile" | "exp" | "currentSal" | "expectedSal" | "status" | "followUp" | "interview" | "owner" | "lastActivity"
  | "email" | "whatsapp" | "altEmail" | "location" | "source" | "notice" | "salesExp" | "callingExp" | "reExp" | "remarks"
  | "sopMet" | "contactedBy" | "inviteSent" | "acknowledged" | "arrivedOnTime"
  | "personality" | "confidence" | "communication" | "behaviour" | "grasping" | "listening" | "negotiation" | "cultureFit"
  | "highestProduct" | "dealValue" | "avgIncentive" | "incentiveConsistent" | "callsPerDay" | "connectedCalls" | "lastSaleDate"
  | "hrDecision" | "hrRemarks" | "finalStatus";
...
// Value getter per EXTRA_COLS key — keep keys in sync with EXTRA_COLS above.
const EXTRA_GET: Record<string, (c: Candidate) => string | null> = {
  email: c => c.email, whatsapp: c => c.whatsappPhone, altEmail: c => c.altEmail, location: c => c.location,
  source: c => c.source, notice: c => c.noticePeriod,
  salesExp: c => c.salesExperience, callingExp: c => c.callingExperience, reExp: c => c.realEstateExperience, remarks: c => c.remarks,
  sopMet: c => c.sopMet, contactedBy: c => c.contactedBy, inviteSent: c => c.inviteMailSent, acknowledged: c => c.candidateAcknowledged, arrivedOnTime: c => c.arrivedOnTime,
  personality: c => c.scorePersonality, confidence: c => c.scoreConfidence, communication: c => c.scoreCommunication, behaviour: c => c.scoreBehaviour,
  grasping: c => c.scoreGrasping, listening: c => c.scoreListening, negotiation: c => c.scoreNegotiation, cultureFit: c => c.scoreCultureFit,
  highestProduct: c => c.highestProductSold, dealValue: c => c.dealValue, avgIncentive: c => c.avgIncentive, incentiveConsistent: c => c.incentiveConsistent,
  callsPerDay: c => c.callsPerDay, connectedCalls: c => c.connectedCallsPerDay, lastSaleDate: c => c.lastSaleDate,
  hrDecision: c => c.hrDecision, hrRemarks: c => c.hrDetailedRemarks, finalStatus: c => c.finalStatus,
};
...
const HF_COLS: HFCol[] = [
  ...
  // All the toggleable Excel fields — plain text sort/filter.
  ...EXTRA_COLS.map(([key]) => ({ key: key as ColKey, kind: "text" as ColKind, str: (c: Candidate) => EXTRA_GET[key](c) ?? "—" })),
];
...
export default function HRCandidateTable({ candidates, agents, countMap = {}, serverSearch = "", perms = NO_PERMS, showClosed = false }: Props) {
  const now = new Date();
  ...
  // On load, honour a ?chip= param — e.g. arriving from the "Closed" chip, which
  // reloads with ?closed=1&chip=closed so the closed rows are actually fetched
  // AND the closed quick-filter is pre-selected.
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    const out = candidates.filter(c => {
      ...
      if (q && !(
        ...
        (c.location ?? "").toLowerCase().includes(q) || (c.remarks ?? "").toLowerCase().includes(q) ||
  ...
  // Closed candidates hidden by the active view (drives the empty-state hint +
  // its "Show closed" shortcut). Zero once we're already showing closed.
  const hiddenClosedCount = showClosed ? 0 : Object.entries(countMap).reduce((a, [k, v]) => a + (CLOSED_SET.has(k) ? v : 0), 0);
  ...
  function exportHref(which: "all" | "filtered" | "selected"): string {
    ...
    if (which === "selected") {
      ...
      // ALSO carry the current URL's server-side segment filters (status/source/
      // owner/date) so a report/funnel drill-down exports exactly that segment (#57).
      if (typeof window !== "undefined") {
        const cur = new URLSearchParams(window.location.search);
        for (const k of ["status", "source", "ownerId", "from", "to"]) {
          const v = cur.get(k); if (v && !params.has(k)) params.set(k, v);
        }
      }
    }
  ...
  const RowActions = ({ c }: { c: Candidate }) => (
    <div className="flex items-center gap-0.5" onClick={stop}>
      {/* Bigger tap targets (p-1.5) + explicit aria-labels, not title-only — audit #126/#130. */}
      {c.phone && <a href={`tel:${c.phone}`} onClick={stop} title="Call" aria-label={`Call ${c.name}`} className="p-1.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400"><Phone className="w-3.5 h-3.5" /></a>}
      {(c.whatsappPhone ?? c.phone) && <a href={waHref(c.whatsappPhone ?? c.phone)} target="_blank" rel="noopener noreferrer" onClick={(e) => { stop(e); e.preventDefault(); openWhatsApp(c.whatsappPhone ?? c.phone); }} title="WhatsApp" aria-label={`WhatsApp ${c.name}`} className="p-1.5 rounded hover:bg-green-50 dark:hover:bg-green-900/30 text-green-600 dark:text-green-400"><MessageCircle className="w-3.5 h-3.5" /></a>}
      {c.email && <a href={`mailto:${c.email}`} onClick={stop} title="Email" aria-label={`Email ${c.name}`} className="p-1.5 rounded hover:bg-rose-50 dark:hover:bg-rose-900/30 text-rose-600 dark:text-rose-400"><Mail className="w-3.5 h-3.5" /></a>}
      <Link href={`/hr/candidates/${c.id}?do=interview`} onClick={stop} title="Schedule Interview" aria-label={`Schedule interview for ${c.name}`} className="p-1.5 rounded hover:bg-purple-50 dark:hover:bg-purple-900/30 text-purple-600 dark:text-purple-400"><Target className="w-3.5 h-3.5" /></Link>
      <Link href={`/hr/candidates/${c.id}?do=note`} onClick={stop} title="Add Note" aria-label={`Add note for ${c.name}`} className="p-1.5 rounded hover:bg-amber-50 dark:hover:bg-amber-900/30 text-amber-600 dark:text-amber-400"><StickyNote className="w-3.5 h-3.5" /></Link>
      <Link href={`/hr/candidates/${c.id}`} onClick={stop} title="Open" aria-label={`Open ${c.name}`} className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-400"><ExternalLink className="w-3.5 h-3.5" /></Link>
    </div>
  );
  ...
  //
  // CRITICAL: each column's WIDTH must equal the delta to the next column's
  // `left`, or the sticky columns drift out of the offsets — a too-narrow Created
  // leaves a gap before Candidate, and a too-wide Candidate lets the first
  // non-sticky column (Designation) show through / get clipped. So every sticky
  // column is pinned to a fixed width (w == min-w == max-w) that matches its
  // offset math: sel 0→32, created 32→136 (104px), candidate 136→304 (168px),
  // phone 304→416 (112px). The widths live here so BOTH the <th> and <td> (which
  // both spread `STICKY.<col>`) stay in lockstep.
  // Created is toggleable — when it's hidden the downstream `left`s shift back by
  // its 104px (else Candidate would stick 104px too far right, leaving a gap). All
  // options are LITERAL class strings so the Tailwind JIT still generates them.
  const createdShown = visible("createdDate");
  const STICKY = {
    sel: "left-0 w-8",
    created: "left-8 w-[104px] min-w-[104px] max-w-[104px]",
    candidate: `${createdShown ? "left-[136px]" : "left-8"} w-[168px] min-w-[168px] max-w-[168px]`,
    phone: `${createdShown ? "left-[304px]" : "left-[200px]"} w-[112px] min-w-[112px] max-w-[112px]`,
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Filters + view toggle. The refine box is deliberately SECONDARY (smaller,
          muted) so it doesn't read as a duplicate of the primary all-pages search
          above — it only narrows the rows already loaded on this page (audit #124). */}
      <div className="flex gap-2 flex-wrap items-center">
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Refine loaded rows…"
          title="Narrows only the rows already on this page. Use the big Search box above to search all candidates."
          className="w-full sm:w-64 border border-gray-200 rounded-lg px-3 py-1.5 text-[13px] bg-gray-50/70 dark:bg-slate-800/60 focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20 dark:border-slate-600" />
      ...
      <div className="flex flex-wrap items-center gap-1.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2">
        ...
        {PRESETS.map(p => (
          <button key={p.name} type="button" onClick={() => applySnapshotObj(p.build())}
            className="text-xs px-2.5 py-1 rounded-full font-semibold whitespace-nowrap bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 dark:text-indigo-200 dark:border-indigo-400/40 dark:hover:bg-indigo-500/20 inline-flex items-center gap-1 transition"
            title={`Preset: ${p.name}`}>
      ...
      {/* Quick filter — a single dropdown instead of a full row of chips. */}
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-gray-500 dark:text-slate-400 shrink-0">Quick filter</span>
        <select
          value={chip}
          onChange={(e) => {
            const k = e.target.value;
            // Closed candidates are NOT loaded in the active view (the server
            // excludes them), so a client-side "closed" filter would match
            // nothing. Reload with ?closed=1 so they actually appear, and
            // carry chip=closed so the closed filter is pre-selected.
            if (k === "closed" && !showClosed) {
              const params = new URLSearchParams(searchParams?.toString() ?? "");
              params.set("closed", "1");
              params.set("chip", "closed");
              params.delete("page");
          ...
          className="text-sm border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-1.5 bg-white dark:bg-slate-900 text-gray-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20 min-w-[220px]"
        >
          {CHIPS.map(([k, label]) => (
            <option key={k} value={k}>{label}{(() => { const cnt = chipCount(k, countMap, showClosed); return cnt != null ? ` (${cnt})` : ""; })()}</option>
          ))}
        </select>
        {chip !== "all" && (
          <button type="button" onClick={() => setChip("all")}
            className="text-xs text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 underline underline-offset-2">
            Clear
          </button>
        )}
      </div>
      ...
      {showAdv && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            <div><label className={lbl}>Designation</label><input className={inp} value={fProfile} onChange={e => setFProfile(e.target.value)} placeholder="e.g. Sales" /></div>
      ...
      {view === "table" && (
        <div className="hidden sm:block overflow-x-auto overflow-y-auto max-h-[72vh] rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900">
          <table className="min-w-[900px] w-full text-xs border-separate border-spacing-0">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={dynHeaders.length + 4} className="px-4 py-10 text-center text-gray-400 text-xs">
                  No candidates match these filters.
                  <div className="mt-2 flex items-center justify-center gap-2">
                    <button type="button" onClick={resetAdv} className="text-xs px-3 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800">Reset filters</button>
                    <a href="/hr/candidates/new" className="text-xs px-3 py-1.5 rounded-lg bg-[#1a2e4a] text-white hover:bg-[#243d60]">+ Add candidate</a>
                  </div>
                  {hiddenClosedCount > 0 && (
                    <div className="mt-2">
                      <Link href="/hr/candidates?closed=1&chip=closed" className="inline-flex items-center gap-1 text-[#1a2e4a] dark:text-blue-400 font-semibold hover:underline">
                        Show {hiddenClosedCount} closed candidate{hiddenClosedCount !== 1 ? "s" : ""} →
                      </Link>
                      <div className="text-gray-400 mt-0.5">Closed candidates (Joined, Not Responding, Switch Off, Rejected…) are hidden by default.</div>
                    </div>
                  )}
                </td></tr>
              )}
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id}
                    ...
                    <td className={`px-2 py-1.5 overflow-hidden sticky ${STICKY.candidate} z-[5] ${sel ? "bg-amber-50/40 dark:bg-amber-900/10" : stickyBg} border-b border-gray-100 dark:border-slate-800`}>
                      <div className="flex items-center gap-1">
                        <Link href={`/hr/candidates/${c.id}`} onClick={stop} className="font-semibold text-[#1a2e4a] dark:text-blue-400 hover:underline truncate max-w-[140px]">{c.name}</Link>
                        <UnreadBadges c={c} />
                      </div>
                      {c.currentCompany && <div className="text-[10px] text-gray-400 truncate max-w-[150px]">{c.currentCompany}</div>}
                    </td>
                    ...
                    {EXTRA_COLS.map(([key]) => visible(key) && (
                      <td key={key} className="px-2 py-1.5 text-gray-600 max-w-[160px] truncate border-b border-gray-100 dark:border-slate-800" title={EXTRA_GET[key](c) ?? undefined}>{EXTRA_GET[key](c) ?? "—"}</td>
                    ))}
      ...
      <div className={`${view === "cards" ? "grid" : "sm:hidden grid"} grid-cols-1 md:grid-cols-2 gap-2`}>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-8 col-span-full space-y-2">
              <div className="text-gray-400 text-sm">No candidates match these filters.</div>
