...
import {
  ...
  Menu, X, Briefcase, LogOut, Upload, Columns3,
} from "lucide-react";
...
const NAV: NavItem[] = [
  ...
  { href: "/hr/board",       label: "Board",      Icon: Columns3 },
    ...
    <div className="flex h-screen overflow-hidden bg-[#f5f6fa] dark:bg-slate-950">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-[#0b1a33] text-white shrink-0">
          <button onClick={() => setMobileOpen(true)} aria-label="Open menu" className="p-2.5 -ml-1 rounded hover:bg-white/10">
        ...
        {/* Mobile bottom nav */}
        <nav className="lg:hidden flex border-t border-gray-200 bg-white dark:bg-slate-900 dark:border-slate-700 shrink-0 safe-bottom">
          {BOTTOM_NAV.map(item => {
            const active = isActive(item.href);
            const badge = badgeFor(item.href);
            return (
