...
export default function HRUserManager({ initialUsers, meId }: { initialUsers: U[]; meId: string }) {
  ...
  async function patch(id: string, data: object) {
    ...
    const res = await fetch("/api/hr/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, ...data }) });
    const json = await res.json().catch(() => ({}));
    setBusy(false);
    if (!res.ok) { setErr(json.error ?? "Couldn't update this user."); return; }
    // Tell the admin when deactivating / un-HR-flagging returned candidates to Unassigned (#43).
    if (json.reassigned > 0) setNote(`${json.reassigned} candidate${json.reassigned === 1 ? "" : "s"} returned to the Unassigned pool.`);
    router.refresh();
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {note && <div className="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded p-2 dark:bg-emerald-900/20 dark:border-emerald-700 dark:text-emerald-300">{note}</div>}
      {err && !showCreate && <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-2 dark:bg-red-900/20 dark:border-red-700">{err}</div>}
