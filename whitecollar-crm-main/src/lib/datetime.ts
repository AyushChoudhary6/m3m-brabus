...
/**
 * The UTC instant for `hh:mm` IST on the IST calendar day of `base` (optionally
 * shifted by `dayOffset` IST days). Use instead of `new Date(base); setHours(hh)`,
 * which resolves to the SERVER's timezone — on Vercel (UTC) that lands ~5.5h off.
 * e.g. day-before-10am-IST reminder = istClockOnDay(scheduledAt, 10, 0, -1).
 */
export function istClockOnDay(base: Date, hh: number, mm = 0, dayOffset = 0): Date {
  const IST = 5.5 * 60 * 60 * 1000;
  const ist = new Date(base.getTime() + IST);
