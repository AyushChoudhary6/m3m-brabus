...
// ── Lifecycle transition policy (audit #86) ──────────────────────────────────
// Previously any status could jump to any status. This adds a light guardrail: it's
// generous WITHIN active statuses (recruiters correct freely) and ACROSS closed reasons
// (re-categorise a close), and always allows closing. It blocks only the nonsensical
// case of teleporting a CLOSED candidate straight into a LATE active stage — you must
// reopen them into early pipeline first. Unknown/legacy statuses are always allowed so
// old records are never trapped.
const REOPEN_ACTIVE_KEYS: HRCandidateStatus[] = ["NEW", "NOT_CALLED", "INTERESTED", "PIPELINE"];
const isClosedKey = (s: string) => (CLOSED_STATUS_KEYS as string[]).includes(s);
const isKnownActiveKey = (s: string) => ACTIVE_STATUS_DEFS.some(d => d.key === s);

/** True if a candidate may move directly from `from` → `to`. */
export function isAllowedStatusTransition(from: string | null | undefined, to: string): boolean {
  if (!from || from === to) return true;
  if (isClosedKey(from) && isKnownActiveKey(to) && !(REOPEN_ACTIVE_KEYS as string[]).includes(to)) {
    return false; // closed → late-active teleport
  }
  return true;
}

/** Human-readable reason for a blocked transition. */
export function statusTransitionError(from: string, to: string): string {
  return `Can't move a "${statusLabel(from)}" candidate straight to "${statusLabel(to)}". Reopen them first (set to New / Interested / Pipeline), then advance.`;
}
