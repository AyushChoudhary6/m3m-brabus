...
export async function GET(req: NextRequest) {
  ...
  const [candidate, templates] = await Promise.all([
    prisma.hRCandidate.findFirst({
      ...
      select: {
        ...
        expectedSalary: true, currentProfile: true, location: true,
      },
    }),
  ...
  const ctx: TemplateContext = {
    ...
    // {{project}} → designation, {{city}} → current location.
    project: candidate.currentProfile
      ? { name: candidate.currentProfile, city: candidate.location ?? "" }
      : (candidate.location ? { name: "", city: candidate.location } : null),
  };
