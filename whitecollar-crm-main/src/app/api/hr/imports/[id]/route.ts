...
import { requireHrPermission, hrApiAuth, hrRoleOf } from "@/lib/hrAccess";
import { prisma } from "@/lib/prisma";

// A batch is visible/editable only to the HR user who ran it — or any ADMIN.
// Prevents one recruiter reading/patching another's import error report (#40).
function batchScope(me: { id: string }, role: string | null) {
  return role === "ADMIN" ? {} : { importedById: me.id };
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const owned = await prisma.hRImport.findFirst({ where: { id, ...batchScope(me, hrRoleOf(me)) }, select: { id: true } });
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { me } = access;
  const { id } = await params;
  const rec = await prisma.hRImport.findFirst({ where: { id, ...batchScope(me, hrRoleOf(me)) } });
