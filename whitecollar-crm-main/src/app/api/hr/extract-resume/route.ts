...
export async function POST(req: NextRequest) {
  ...
  // Per-user throttle — this endpoint ships candidate PII to a third party (Anthropic)
  // and costs money; cap abuse at ~30 extractions / 5 min per user (audit #100).
  const rl = isRateLimited(`hr:extract:${auth.me.id}`, 30, 5 * 60 * 1000);
  if (rl.limited) return NextResponse.json({ error: `Too many resume extractions — retry in ${rl.retryAfterSec}s.` }, { status: 429 });
