...
import { closeFollowUpsIfTerminal, syncNextActionDate } from "@/lib/hrFollowups";
import { istClockOnDay } from "@/lib/datetime";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Candidate name for the interviewer-facing reminder (#79).
  const cand = await prisma.hRCandidate.findUnique({ where: { id }, select: { name: true } });
  ...
  const [interview] = await prisma.$transaction(async (tx) => {
    ...
    // Auto-create day-before confirmation follow-up (10:00 IST the day before).
    const dayBefore = istClockOnDay(scheduledAt, 10, 0, -1);
    ...
    // Auto-create morning-of reminder (08:00 IST on the interview day), owned by the
    // INTERVIEWER (not the scheduler) so it lands in the person conducting the interview's
    // own follow-up queue — previously the assigned interviewer was never surfaced (#79).
    const morning = istClockOnDay(scheduledAt, 8, 0, 0);
    if (morning > new Date()) {
      await tx.hRFollowUp.create({
        data: {
          candidateId: id, userId: interviewerId,
          type: "REMINDER",
          dueAt: morning,
          notes: `Interview today — ${candName}, ${itype.replace(/_/g, " ")} at ${scheduledAt.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Kolkata" })}`,
          autoCreated: true,
        },
      });
    }
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "reschedule" || (body.scheduledAt && body.action !== "result")) {
    ...
    const iv = await prisma.$transaction(async (tx) => {
      const updated = await tx.hRInterview.update({
        ...
        data: {
          ...
          // Mark the row RESCHEDULED (the enum value was previously never written) so
          // a moved interview is distinguishable from a fresh one (audit #46).
      ...
      const dayBefore = istClockOnDay(newAt, 10, 0, -1);
      if (dayBefore > new Date()) {
        await tx.hRFollowUp.create({
          data: {
      ...
      const morning = istClockOnDay(newAt, 8, 0, 0);
      if (morning > new Date()) {
        await tx.hRFollowUp.create({
          data: {
    ...
    // The auto follow-ups were deleted+recreated — repoint nextActionDate at the
    // soonest open follow-up so the candidate keeps a correct next action (#36).
    await syncNextActionDate(candidateId);
  ...
  if (body.action === "result" || body.recommendation) {
    ...
    const iv = await prisma.$transaction(async (tx) => {
      ...
      // NOTE: feedback (result + notes) is stored on the HRInterview row above.
      // The candidate table has no `interviewFeedback` column — writing it here
      // threw a PrismaClientValidationError and rolled back the whole result
      // capture (audit #4/#25). Store status only.
    ...
    // A terminal outcome (REJECTED) must close open follow-ups so the candidate
    // leaves the Call-Now queue / Overdue KPIs (#26).
    await closeFollowUpsIfTerminal(candidateId, recommendation ? RECOMMENDATION_STATUS[recommendation] : "INTERVIEW_HELD");
  ...
  if (body.attendanceStatus === "ATTENDED") {
    ...
    // Mark candidate No Show and auto-create a recovery follow-up for tomorrow 10:00 IST.
    const recovery = istClockOnDay(new Date(), 10, 0, 1);
    await prisma.$transaction([
      prisma.hRCandidate.update({
        where: { id: candidateId },
        data: { status: "NO_SHOW", nextActionDate: recovery },
      }),
      prisma.hRActivity.create({
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Deleting the interview removed its follow-ups — resync the candidate's
  // next-action pointer to the soonest remaining open follow-up (#36).
  await syncNextActionDate(candidateId);
