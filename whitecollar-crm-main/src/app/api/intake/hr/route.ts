...
export async function POST(req: NextRequest) {
  ...
  // Abuse throttle for this internet-facing endpoint (audit #100): ~60 submissions
  // per 5 min per key+IP. Legit form traffic is well under this; floods are blocked.
  const rl = isRateLimited(`intake:hr:${apiKey}:${ip ?? "noip"}`, 60, 5 * 60 * 1000);
  if (rl.limited) return fail(429, `Too many submissions — retry in ${rl.retryAfterSec}s.`);
  ...
  try {
    ...
    // Optional ROUND-ROBIN distribution across active HR recruiters (audit #81), so
    // website intake isn't all dumped on one owner. When enabled it overrides the
    // single default owner; the rotating index is persisted in Settings.
    if ((await getSetting("hr.intakeRoundRobin")) === "1") {
      const pool = await prisma.user.findMany({
        where: { active: true, role: { not: "ADMIN" }, OR: [{ hrOnly: true }, { hrTeam: true }] },
        select: { id: true }, orderBy: { id: "asc" },
      });
      if (pool.length > 0) {
        const idx = (parseInt((await getSetting("hr.intakeRoundRobinIdx")) || "0", 10) || 0) % pool.length;
        ownerId = pool[idx].id;
        await setSetting("hr.intakeRoundRobinIdx", String((idx + 1) % pool.length));
      }
    }
    ...
    const existing = dupWhere
      ? await prisma.hRCandidate.findFirst({ where: { AND: [dupWhere, { deletedAt: null }] }, select: { id: true, status: true, remarks: true } })
      : null;
    ...
    if (existing) {
      ...
      if (d.remarks && d.remarks.trim()) {
        const mergedRaw = mergeRawRemark(existing.remarks, d.remarks, `re-application · ${d.source}`);
        await prisma.hRCandidate.update({ where: { id: existing.id }, data: { remarks: mergedRaw } }).catch(() => {});
      }
