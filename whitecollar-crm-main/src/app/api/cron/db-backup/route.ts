...
export async function GET(req: NextRequest) {
  ...
  // ── HR module tables (audit #12/#105 — previously omitted ENTIRELY). ──
  // HRResume stores base64 bytes in `url`; we back up METADATA only so the JSON
  // stays bounded within the 30s/size budget.
  const [hrCandidates, hrInterviews, hrFollowUps, hrRemarks, hrResumes] = await Promise.all([
    prisma.hRCandidate.findMany(),
    prisma.hRInterview.findMany(),
    prisma.hRFollowUp.findMany(),
    prisma.hRRemark.findMany(),
    prisma.hRResume.findMany({ select: { id: true, candidateId: true, filename: true, mimeType: true, isActive: true, uploadedById: true, createdAt: true } }),
  ]);
  const hrActivities = cutoff
    ? await prisma.$queryRaw<unknown[]>`SELECT * FROM "HRActivity" WHERE "createdAt" >= ${cutoff} ORDER BY "createdAt" DESC`
    : await prisma.$queryRaw<unknown[]>`SELECT * FROM "HRActivity" ORDER BY "createdAt" DESC`;
  ...
  const payload = jsonSafe({
    ...
    hrCandidateCount: hrCandidates.length,
    hrActivityCount: hrActivities.length,
    tables: {
      users,
      projects,
      units,
