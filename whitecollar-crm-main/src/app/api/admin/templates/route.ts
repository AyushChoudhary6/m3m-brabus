...
export async function POST(req: NextRequest) {
  ...
  const visibleToHr = body.visibleToHr === undefined ? true : Boolean(body.visibleToHr); // audit #41
  const t = await prisma.template.create({
    data: { kind: kind as TemplateKind, trigger: trigger as TemplateTrigger, name, subject, body: text, visibleToHr, createdById: me.id },
  });
