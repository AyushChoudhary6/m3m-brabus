...
export default async function MissedPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  // "Overdue" = due BEFORE the start of today IST (day-granular), matching the
  // dashboard — a follow-up due later today is not overdue (audit #8/#27).
  const todayStart = istDayRange().start;
  const now = new Date(); // point-in-time, for "upcoming" (future) interviews
  const filter = sp.filter ?? "all";

  // Scope to candidates the user may see, not soft-deleted, AND not closed (#37).
  ...
  const [overdueFollowUps, noNextAction, noShowsPending, pendingConfirm] = await Promise.all([
    prisma.hRFollowUp.findMany({
      where: { completedAt: null, dueAt: { lt: todayStart }, candidate: candidateScope },
      orderBy: { dueAt: "asc" },
      take: 50,
  ...
  // REAL totals for the pills (independent of the take: caps above) so the numbers
  // never report a truncated fetch length (audit #147).
  const [overdueTotal, noNextTotal, noShowTotal, confirmTotal] = await Promise.all([
    prisma.hRFollowUp.count({ where: { completedAt: null, dueAt: { lt: todayStart }, candidate: candidateScope } }),
    prisma.hRCandidate.count({ where: { AND: [hrScopeWhere(me), { deletedAt: null, nextActionDate: null, status: { notIn: CLOSED_STATUS_KEYS as never[] } }] } }),
    prisma.hRInterview.count({ where: { attendanceStatus: "NO_SHOW", candidate: candidateScope } }),
    prisma.hRInterview.count({ where: { scheduledAt: { gte: now }, confirmationStatus: "PENDING", candidate: candidateScope } }),
  ]);

  const sections: Array<{ id:string; Icon:LucideIcon; label:string; count:number; urgent:boolean }> = [
    { id:"overdue",  Icon:AlertTriangle, label:"Overdue Follow-Ups",          count: overdueTotal,  urgent: overdueTotal > 0 },
    { id:"no-next",  Icon:HelpCircle,    label:"Candidates Without Next Action", count: noNextTotal,   urgent: false },
    { id:"no-show",  Icon:Ban,           label:"No-Show Recovery Needed",       count: noShowTotal,   urgent: noShowTotal > 0 },
    { id:"confirm",  Icon:CheckCircle2,  label:"Pending Interview Confirmation", count: confirmTotal, urgent: confirmTotal > 0 },
  ];
