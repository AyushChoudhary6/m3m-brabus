...
export default async function InterviewsPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  // Scope to candidates the user may see, exclude soft-deleted, AND exclude closed/rejected
  // candidates — every filter here is an actionable queue, so a rejected candidate's stale
  // interview must not appear (it would show a Rejected badge in a needs-action row).
  let where: NonNullable<Parameters<typeof prisma.hRInterview.findMany>[0]>["where"] = {
    candidate: { AND: [hrScopeWhere(me), { deletedAt: null, status: { notIn: CLOSED_STATUS_KEYS } }] },
  };
  ...
  const [interviews, total] = await Promise.all([
    ...
    prisma.hRInterview.count({ where }), // real total so truncation is never silent (#146/#127)
  ]);
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {interviews.map(iv => (
                  <tr key={iv.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-4 py-3">
                      <HRInterviewRowActions interviewId={iv.id} candidateId={iv.candidateId} phone={iv.candidate.phone} attendanceStatus={iv.attendanceStatus} confirmationStatus={iv.confirmationStatus} scheduledAt={iv.scheduledAt.toISOString()} result={iv.result} recommendation={iv.recommendation} />
                    </td>
                  </tr>
                ))}
              </tbody>
